...
trait FileReaderState {}

impl FileReaderState for Idle {}
impl FileReaderState for Open {}
impl FileReaderState for Scan {}
impl FileReaderState for Closed {}

struct FileReader<T> {
    _p: PhantomData<T>,
}

impl FileReader<Idle> {
    fn open(self) -> FileReader<Open> {
        FileReader { _p: PhantomData }
    }
}
...
pub trait FileOpener: Send + Unpin {
    ...
    /// of [`RecordBatch`]
    fn open(&self, file_meta: ObjectMeta, range: Option<Range<i64>>) -> Result<FileOpenFuture, ()>;
}
...
pub enum State {
    Idle(FileReader<Idle>),
    Open(FileReader<Open>),
    Scan(FileReader<Scan>),
    Closed(FileReader<Closed>),
}

impl State {
    fn new() -> Self {
        Self::Idle(FileReader { _p: PhantomData })
    }
}

pub struct LogicalFile {}

pub struct PresignedFileStream {
    files: Vec<LogicalFile>,
    state: State,
}

impl PresignedFileStream {
    pub fn new(files: Vec<LogicalFile>) -> Self {
        Self {
            files,
            state: State::Idle(FileReader { _p: PhantomData }),
        }
    }
